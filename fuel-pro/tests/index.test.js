...
//   it('should create or update a user profile and respond with the profile data', async () => {
//     // Mock user session data
//     const mockUserSession = { user: 'test_user' };
...
//     // Check if the profile is created or updated in the database
//     const userProfile = await prisma.profile.findUnique({
//       where: {
//         clientUsername: mockUserSession.user
//       }
//     });
//     expect(userProfile).toBeTruthy(); // Check if the user profile exists in the database
//     expect(userProfile.firstName).toBe(mockRequestBody.firstName); // Check if the first name matches
//     expect(userProfile.lastName).toBe(mockRequestBody.lastName); // Check if the last name matches
//     // Add more assertions for other profile properties if needed
//   });
// });
...
describe('POST /signup', () => {
  it('should respond with status 400 if user already exists', async () => {
    ...
    prisma.user.findUnique.mockResolvedValueOnce({ username: 'Aryan' });

    // Make the request to the endpoint using supertest
    const response = await request(app).post('/signup').send({ username: 'Aryan', password: 'password' });
  ...
  it('should respond with status 200 and the new user\'s username if user is successfully created', async () => {
    // Mock the new user creation
    prisma.user.findUnique.mockResolvedValueOnce(null); // No existing user
    prisma.user.create.mockResolvedValueOnce({ username: 'Aryan' }); // New user created successfully
    ...
    // Assert the response
    expect(response.status).toBe(200);
    expect(response.body).toEqual('Aryan');
  });

  it('should respond with status 500 if an error occurs during user creation', async () => {
    // Mock the error during user creation
    prisma.user.findUnique.mockResolvedValueOnce(null); // No existing user
    prisma.user.create.mockRejectedValueOnce(new Error('Database error')); // Error during user creation
...
//     const newUser = { username: 'Aryan', password: 'Aar120' };
